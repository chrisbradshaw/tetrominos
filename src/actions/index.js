...
export const rotateTetromino = () => ({
  function(dispatch, getState) {
    ...
    if (
      !checkCollisions('rotation', activeTetrominos, rotatedTetromino) &&
      gameStatus === 'PLAYING'
    ) {
...
export const moveTetromino = direction =>
  function(dispatch, getState) {
    ...
    const collisionCheck = checkCollisions(
      direction,
      activeTetrominos,
      currentTetromino
    );
    ...
    switch (direction) {
      ...
      case 'down':
        if (collisionCheck === false) {
          ...
          const clearedLines = getCompletedLines(
            activeTetrominos,
            currentTetromino
          ).length;
...
function dropTetromino(dispatch, startTime, getState) {
  ...
  requestAnimationFrame(
    dropTetromino.bind(this, dispatch, startTime, getState)
  );
}
